<template>
  <div class="user-addresslist-page">
    <div class="address-list-box">
      ...
      <loading v-else :containerSize="'huge'"></loading>
    </div>
    <transition name="fade-modal">
      <modal
        @closeModal="onCloseModal"
        v-if="showModalNewAddress"
        :modalWidth="660"
        :modalHeight="432"
        :modalTitle="'添加地址'"
        model="out-in"
        ...
        <div class="form-container">
          <div class="add-form clearfix">
            <div class="section section-head">
              <input type="text" placeholder="姓名" v-model="name" class="input-name" @input="checkFormData('name')"/>
              <input type="text" placeholder="电话" v-model="phone" @input="checkFormData('phone')"/>
            </div>
            <div class="section section-district">
              <input type="text" v-model="district" placeholder="选择省/市/区/街道" v-on:focus="showSearchDistrict" />
              <i class="iconfont iconarrowdown"></i>

              <search-district
                v-if="isShowSearchDistrict"
                v-on:showChoose="showChooseDistrict"
                v-on:closeForm="handleCloseForm"
              ></search-district>

              <choose-district v-if="isShowChooseDistrict" @closeCd="handleCloseCd" @showSd="showSearchDistrict" @selectDistrict="getDistrict"></choose-district>
            ...
            <div class="section section-tag">
              <input type="text" v-model="tag" placeholder="地址标签" @input="checkFormData('tag')" />
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      showModalNewAddress: false, //显示添加地址模态框 
      showModalUpdateAddress: false, //显示修改地址模态框
      deleteStatus: 0, //删除地址显示状态 0未请求 1正在请求 2请求成功 3删除失败
      isShowSearchDistrict: false, //是否显示搜索街区框
      isShowChooseDistrict: false, //是否显示手动选择街区框
      userAddresses: [], //
      selectAdId: 0,//选中地址id
      // v-model 绑定数据
      name: "",
  ...
  methods: {
    onCloseModal() {
      this.showModalNewAddress = false;
      this.showModalUpdateAddress = false;
    },
    addAddress() {
      this.showModalNewAddress = true;
    },
    ...
    //选择收货地址
    chooseAddress(index){
      this.selectAdId = this.userAddresses[index].id;
      this.$emit('selectAddress',this.userAddresses[index]);
    },
    handleCloseForm() {
